...
namespace atum8
{
    ...
    Mecanum::Mecanum(UPMotorGroup iRFMotor,
    ...
    okapi::QLength Mecanum::getDistance() const
    {
        ...
        avgRotation += rFMotor->get_positions()[0];
        avgRotation += lFMotor->get_positions()[0];
        avgRotation += lBMotor->get_positions()[0];
        avgRotation += rBMotor->get_positions()[0];
        avgRotation /= 4;
    ...
    {
        const double lAvgRotation{(lFMotor->get_positions()[0] + lBMotor->get_positions()[0]) / 2};
        const double rAvgRotation{(rFMotor->get_positions()[0] + rBMotor->get_positions()[0]) / 2};
    ...
    {
        rFMotor->set_brake_modes(brakeMode);
        lFMotor->set_brake_modes(brakeMode);
        lBMotor->set_brake_modes(brakeMode);
        rBMotor->set_brake_modes(brakeMode);
    }
    ...
    {
        return std::make_shared<Mecanum>(std::make_unique<pros::MotorGroup>(rFPort),
                                         std::make_unique<pros::MotorGroup>(lFPort),
                                         std::make_unique<pros::MotorGroup>(lBPort),
                                         std::make_unique<pros::MotorGroup>(rBPort),
                                         dimensions,
                                         driverSettings,
    ...
    SPMecanumBuilder SPMecanumBuilder::withRFMotor(const std::vector<std::int8_t> &iPorts)
    {
        rFPort = iPorts;
        return *this;
    }

    SPMecanumBuilder SPMecanumBuilder::withLFMotor(const std::vector<std::int8_t> &iPorts)
    {
        lFPort = iPorts;
        return *this;
    }

    SPMecanumBuilder SPMecanumBuilder::withLBMotor(const std::vector<std::int8_t> &iPorts)
    {
        lBPort = iPorts;
        return *this;
    }

    SPMecanumBuilder SPMecanumBuilder::withRBMotor(const std::vector<std::int8_t> &iPorts)
    {
        rBPort = iPorts;
        return *this;
    }
