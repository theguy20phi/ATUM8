...
namespace atum8
{
  ...
  Pid aimBotControllerz(50, 0, 0, 5, 0.5);
  ...
  visionSensorGoal.set_signature(redID, &RED_SIG);
    pros::vision_signature_s_t BLUE_SIG = pros::Vision::signature_from_utility(
        blueID, -3449, -2547, -2998, 5161, 9663, 7412, 3.000, 0);
    visionSensorGoal.set_signature(blueID, &BLUE_SIG);
    visionSensorGoal.set_exposure(75);
  ...
  void Drive::tankDrive()
  {
    ...
    if (Chris.get_digital(DIGITAL_R2) && visionSensorGoal.get_object_count())
    {
      pros::vision_object_s_t goal =
          visionSensorGoal.get_by_sig(0, isRedMode ? redID : blueID);
      aimAssistPower =
          aimBotControllerz.getOutput(goal.x_middle_coord, visionFOVWidth * 0.5)/12000 * 127;
          std::cout<< "error: " << goal.x_middle_coord - visionFOVWidth * 0.5 << std::endl;
    }
